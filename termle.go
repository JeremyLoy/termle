...
func parseGuesses() map[string]struct{} {
...
func currentAnswer(day int) string {
...
var valid = regexp.MustCompile(`^[A-Za-z]{5}$`)

//go:embed guesses.txt
//go:embed answers.txt
var files embed.FS

var dayFlag = flag.Int("day", currentDay(), "select a specific wordle by day")
var randomFlag = flag.Bool("random", false, "pick a random wordle")

func randomDay() int {
	return rand.Intn(2315)
}
...
func main() {
	...
	answer := currentAnswer(day)
	validGuesses := parseGuesses()
	b := newBoard()
	currentTurn := 0
	turnsRemaining := 6
